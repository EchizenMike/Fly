...
$result_lognum = mysqli_query($conn, "SELECT lognum FROM user_table WHERE username = '$name'");
if ($result->num_rows > 0) {//判断结果集是否为空
  //  echo "OK" . " ";
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        while($row2 =  mysqli_fetch_assoc($result_lognum)){
        	$resobj->stu_loginnum = $row2['lognum'];
        }  
...
//  $flag['result']="0";
	$inform = new INFORM();
	$uslognum = new USLOGNUM();
	while($row2 =  mysqli_fetch_assoc($result_lognum)){
       	$uslognum->stu_loginnum = $row2['lognum'];
       }  
    $dataresult [] = $uslognum;
    $inform->code = 1;
    $inform->msg = '无内容';
